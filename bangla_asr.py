...
parser.add_argument("--target-dir", default='BenglaASR_dataset/', type=str, help="Directory to store the dataset.")
parser.add_argument('--files-to-use', default="asr_bengali_0.zip,"
                                              "asr_bengali_1.zip,"
                                              "asr_bengali_2.zip", type=str,
...
LIBRI_SPEECH_URLS = {
    ...
    "val": ["https://www.openslr.org/resources/53/asr_bengali_1.zip"],

    "test_clean": ["https://www.openslr.org/resources/53/asr_bengali_2.zip"],
}
...
def main():
    ...
    files_to_dl = args.files_to_use.strip().split(',')
    for split_type, lst_bangla_asr_urls in LIBRI_SPEECH_URLS.items():
        ...
        for url in lst_bangla_asr_urls:
            ...
            transcript_file = os.path.join(split_dir, 'asr_bengali', 'utt_spk_text.tsv')
        ...
        if split_type == 'train':  # Prune to min/max duration
            create_manifest(
                data_path=split_dir,
                output_name='asr_bengali_' + split_type + '_manifest.json',
